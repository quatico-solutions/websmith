...
describe("createStyleCompiler", () => {
    it("yields injected importedStyles method with style import", () => {
        ...
        const result = compileStyles("two.ts", testSystem.readFile("two.ts")!.toString());
        // FIXME: Parse result to find style method
...
display: none;
    ...
    it("yields unmodified code w/o style import", () => {
        ...
        const result = compileStyles("one.ts", testSystem.readFile("one.ts")!.toString());

        expect(result).toBe("class One {}");
    });

    it("yields no method w/o customElement decorator", () => {
        const compileStyles = createStyleCompiler({ reporter, system: testSystem }, {});

        const result = compileStyles("four.ts", testSystem.readFile("four.ts")!.toString());
    ...
    it("yields no method w/ non-existing style import", () => {
        ...
        const result = compileStyles("five.ts", testSystem.readFile("five.ts")!.toString());
