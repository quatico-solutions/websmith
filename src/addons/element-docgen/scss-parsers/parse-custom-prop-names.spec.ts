...
import { isNameEnd, isNamePart, isNameStart, parseCustomPropertyNames, parseNodeValue } from "./parse-custom-prop-names";
...
describe("parseCustomPropertyNames with declaration", () => {
    ...
    it("returns names with custom property declaration with '--' in name", () => {
        const actual = parseCustomPropertyNames(".my-element { --my-expected--one: 100%; }");

        expect(actual).toEqual(["--my-expected--one"]);
    });

    it("returns names with three custom properties in declaration", () => {
        const actual = parseCustomPropertyNames(
    ...
    it("returns names with custom properties in declaration and value", () => {
        const actual = parseCustomPropertyNames(".my-element { --qs-alert__border-radius--type: var(--qs-alert__border-radius--one); }");
    ...
    it("returns names with custom properties in declaration and value of same", () => {
        ...
        expect(actual).toEqual(["--qs-alert__outline--type", "--qs-alert-is-info__outline", "--qs-alert__outline", "--qs-outline"]);
    });
